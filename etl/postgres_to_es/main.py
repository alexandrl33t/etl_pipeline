...
elastic_loader = ElasticLoader(config=ELASTIC_CONFIG, storage=storage)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        for index in indexes:
            if not (load_from := storage.retrieve_state(f"load_from_{index}")):
                load_from = str(datetime.min)
            try:
